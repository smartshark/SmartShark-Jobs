 ...
 * Simple Algorithm which finds duplicate developers from the people collection
...
object algorithm1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val format = udf((x: Double) => x.toDouble)
    ...
    /*
     * Find Potential Match by
     * performing self-join using some equi-join conditions
     */
    ...
    if( args.length>0 ) {
       ...
       * false if not.
       */
      val splitt = joined.randomSplit(Array(0.5, 0.5), seed = 500)
      ...
      sample_1.coalesce(1).write
        ...
        .save(args(0))
          ...
          case t: Throwable => println("Unlabelled file already exists. Continue without overwriting.")
        }
    }
    ...
    val mydf = df.withColumn("f1", jaroWinkler(labeleddata("Prefix"), labeleddata("Prefix_1")))
      .withColumn("f2", jaroWinkler(labeleddata("Firstname"), labeleddata("Firstname_1")))
      .withColumn("f3", jaroWinkler(labeleddata("Lastname"), labeleddata("Lastname_1")))
      .withColumn("f4", format((intersectlen(labeleddata("Prefix"), labeleddata("Prefix_1")) / (length(labeleddata("Prefix")) + length(labeleddata("Prefix_1")) - intersectlen(labeleddata("Prefix"), labeleddata("Prefix_1"))))))
      .withColumn("f5", format((intersectlen(labeleddata("Firstname"), labeleddata("Firstname_1")) / (length(labeleddata("Firstname")) + length(labeleddata("Firstname_1")) - intersectlen(labeleddata("Firstname"), labeleddata("Firstname_1"))))))
      .withColumn("f6", format((intersectlen(labeleddata("Lastname"), labeleddata("Lastname_1")) / (length(labeleddata("Lastname")) + length(labeleddata("Lastname_1")) - intersectlen(labeleddata("Lastname"), labeleddata("Lastname_1"))))))
